...
var D3lib = {
    PhrasesEnglish: new Array('crap', 'dude', 'hacker',
        'hacks', 'you', 'cool', 'oh my god',
        'fear', 'power', 'own', 'lol',
        'what the hell', 'elite', 'for the win',
        'oh really', 'good game'),

    PhrasesLeet: new Array('carp', 'dood', 'haxor', 'hax', 'joo',
        'kewl', 'omg', 'ph43', 'powwah', 'pwn', 'lawl',
        'wth', 'leet', 'ftw', 'o rly', 'gg'),

    LettersEnglish: new Array('n', 'b', 'k', 'd', 'e', 'f', 'g', 'h',
        'p', 'm', 'r', 'l', 'o', 'q', 's', 't',
        'u', 'x', 'w', 'y', 'z', 'c', 'a', 'j',
        'i', 'v', ' '),

    LettersLeet: new Array('/\\/', '|}', '|X', '[)', '3', '|=', 'gee', '|-|',
        '|*', '(\\/)', '|2', '1', '()', '0', '$', '+',
        '|_|', '><', '\\X/', '\'/', '2', '<', '/\\', '_|',
        '|', '\\/', '  '),
    ...
    html_entity_decode: function (string, quote_style) {
        ...
        if (false === (hash_map = D3lib.get_html_translation_table('HTML_ENTITIES', quote_style))) {
            return false;
        }
    ...
    leetEncode: function (inputString) {
        ...
        for (i = 0; i < D3lib.PhrasesEnglish.length; ++i)
            inputString = inputString.replace(
                new RegExp(D3lib.PhrasesEnglish[i], "gi"),
                D3lib.PhrasesLeet[i]
            );

        for (i = 0; i < D3lib.LettersEnglish.length; ++i)
            inputString = inputString.replace(
                new RegExp(D3lib.LettersEnglish[i], "gi"),
                D3lib.LettersLeet[i]
            );
        return inputString;
    },
    ...
    leetDecode: function (inputString) {
        for (i = 0; i < D3lib.LettersLeet.length; ++i)
            inputString = inputString.replace(
                new RegExp(RegExp.escape(D3lib.LettersLeet[i]), "g"),
                D3lib.LettersEnglish[i]
            );

        for (i = 0; i < D3lib.PhrasesLeet.length; ++i)
            inputString = inputString.replace(
                new RegExp(RegExp.escape(D3lib.PhrasesLeet[i]), "g"),
                D3lib.PhrasesEnglish[i]
            );
        return inputString;
    },
    ...
    unserialize: function (data) {
        ...
        try {
            ...
            return D3lib.var_dump(expectType(data, initCache())[0]);
        } catch (err) {
            console.error(err);
            return err;
        }
    ...
    var_dump: function (x, max, sep, l) {
        ...
        if (x === null) {
            ...
            for (i in x) {
                try {
                    r += tab + '[' + i + '] : ' + D3lib.var_dump(x[i], max, sep, (l + 1));
                } catch (e) {
                    return "[ERROR: " + e + "]\n";
                }
            }
